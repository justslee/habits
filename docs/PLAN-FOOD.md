...
2. **Candidate discovery** is one model call with the Responses API's built-in web search (decided 2026-09-10): the prompt carries your standing brief (editable on the Recipes screen), the learned taste weights, the titles already in the catalogue, and the source priority; the model opens the pages and returns recipes already normalised (essential verdicts with reasons, protein, keeps-for, reheat, store and pack estimates). Code still enforces ≤ 12 essential ingredients and ≤ 90 minutes and dedupes by URL. No scraping, no browser.
