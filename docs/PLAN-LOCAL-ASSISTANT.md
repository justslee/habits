...
2. **Local env.** Rewrite `backend/.env`: `DATABASE_URL=sqlite:///./data/mastery.db`, `HABITS_SECRETS_DISABLED=1`, `OAUTH_REDIRECT_BASE` and `ALLOWED_ORIGINS` set to the Tailscale hostname, `TZ=America/New_York`. Pull `OPENAI_API_KEY`, `DEEPGRAM_API_KEY`, `WHOOP_*`, `API_KEY` once from Secrets Manager with the AWS CLI. Keep `API_KEY` identical to the one baked into build 15 so the existing app keeps authenticating.
3. **Whoop redirect.** Add the Tailscale callback URL to the Whoop developer app. Existing tokens in the `oauth_connections` table migrate with the data.
