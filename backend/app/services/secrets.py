...
Production API keys (OPENAI_API_KEY, WHOOP_CLIENT_ID/SECRET, API_KEY, …) live in
a single Secrets Manager secret — a JSON object of NAME -> value, named
"habits/prod" by default. We fetch it once at startup and populate os.environ for
any key not already set, so the rest of the app keeps reading plain
`os.getenv(...)` and an explicit env var / local `.env` always wins.

Mirrors the proven scorecard loader (projects/scorecard app/services/secrets.py),
kept bifurcated via its own default secret name.
...
Requirements for this to populate keys on the EC2 box:
  - boto3 installed (declared in requirements.txt).
...
log = logging.getLogger("habits.secrets")

DEFAULT_SECRET_NAME = "habits/prod"
...
def load_secrets_into_env(secret_name: Optional[str] = None, client: Any = None) -> list[str]:
    ...
    """
    if os.getenv("HABITS_SECRETS_DISABLED") == "1":
        return []

    name = secret_name or os.getenv("HABITS_SECRETS_NAME") or DEFAULT_SECRET_NAME

    try:
        c = client or _default_client()
        resp = c.get_secret_value(SecretId=name)
    except Exception as e:  # noqa: BLE001 — boto missing, no creds, AccessDenied, NotFound …
        log.warning("secrets: could not load %s (%s); continuing with env only", name, type(e).__name__)
        return []
    ...
    if loaded:
        log.info("secrets: loaded %d key(s) from %s: %s", len(loaded), name, ", ".join(sorted(loaded)))
    return loaded
