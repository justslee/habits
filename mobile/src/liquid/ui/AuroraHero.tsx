 ...
 * moves. A gradient over the image keeps white type legible in both appearances. Motion stops
 * when the screen is not focused, when Reduce Motion or Quiet is on, or when the owner presses
 * pause. This is a moving still, not a video — the image is AI-generated, and the info action
 * says so.
 */
...
import { AURORA, fonts } from '../tokens';
import { useTheme } from '../theme';
import { feel } from '../haptics';

const IMAGE = require('../../../assets/aurora-north-star.jpg');
...
export function AuroraHero({
  ...
  return (
    <View style={[s.hero, { minHeight: (compact ? 173 : 350) + insets.top, paddingTop: insets.top + 18 }]}>
      <Animated.View style={[StyleSheet.absoluteFill, back]}>
        <Image source={IMAGE} style={s.photo} resizeMode="cover" accessible
          accessibilityLabel="Photographic-style northern lights above mountains and a still lake; AI-generated" />
      </Animated.View>
      ...
      <LinearGradient
        pointerEvents="none"
        colors={['rgba(6,18,27,0.07)', 'rgba(6,18,27,0.02)', 'rgba(7,20,27,0.85)', 'rgba(12,23,28,0.91)']}
      ...
      <View style={s.top}>
        <View style={s.wordmark}>
          <Ionicons name="sparkles" size={21} color={AURORA.wordmarkStar} />
          <Animated.Text style={s.wordmarkText}>North Star</Animated.Text>
        </View>
        <View style={s.actions}>
          <Pressable
            ...
            style={s.action}
          >
            <Ionicons name={playing ? 'pause' : 'play'} size={16} color={AURORA.actionFg} />
          </Pressable>
          <Pressable
            accessibilityRole="button"
            ...
            style={s.action}
          >
            <Ionicons name="information" size={16} color={AURORA.actionFg} />
          </Pressable>
        </View>
      </View>

      <View style={[s.mantra, { marginTop: compact ? 22 : 79 }]}>
        {eyebrow ? <Animated.Text style={s.eyebrow}>{eyebrow.toUpperCase()}</Animated.Text> : null}
        <Animated.Text
          style={[
            s.title,
        ...
        {caption ? <Animated.Text style={s.caption}>{caption}</Animated.Text> : null}
      </View>
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  wordmarkText: { fontFamily: fonts.serif, fontSize: 28, color: AURORA.fg, letterSpacing: -0.6 },
  actions: { flexDirection: 'row', alignItems: 'center', gap: 7 },
  action: {
    width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center',
    backgroundColor: AURORA.actionBg, borderWidth: 1, borderColor: AURORA.actionLine,
  },
  mantra: { maxWidth: 350 },
  eyebrow: { fontFamily: fonts.regular, fontSize: 11, letterSpacing: 1.65, color: AURORA.eyebrow, marginBottom: 12 },
  title: { fontFamily: fonts.serif, color: AURORA.fg, lineHeight: 32 * 1.13, letterSpacing: -0.6 },
  caption: { fontFamily: fonts.regular, fontSize: 11, color: AURORA.caption, marginTop: 12, lineHeight: 11 * 1.5 },
});
